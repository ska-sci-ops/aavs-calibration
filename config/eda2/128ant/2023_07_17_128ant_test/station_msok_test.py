...
class Station(object):
    ...
    def check_station_status(self):
        ...
        for tile in self.tiles:
           ...
           print("DEBUG : tile_id = %d vs. len = %d" % (tile.get_tile_id(),len(self.tiles)))

        
        tile_ids = []                        
        for tile in self.tiles:
            if tile.tpm is None:
                print("DEBUG : is tile.tpm = None ???")
                self.properly_formed_station = False
                break

            tile_id = tile.get_tile_id()
            print("DEBUG : tile_id = %d vs. len = %d" % (tile.get_tile_id(),len(self.tiles)))
            if tile.get_tile_id() <= 16 and tile_id not in tile_ids:
...
if __name__ == "__main__":
    ...
    # TPM 12 overheats -> needs to have temperature threshold set to 70*C instead of default 65*C :
#    station.tiles[11].tpm.set_shutdown_temperature(70)
