...
do
   ...
   echo "UPDATE calibration_solution set x_amp_fit='{"$amp_x_list"}',y_amp_fit='{"$amp_y_list"}' where fit_time='"$fit_time"' and ant_id=$ant and station_id=$station_id;" >> ${sqlfile}
done
