...
if [[ $generate_sql -gt 0 && -n $fit_time ]]; then
   ...
   echo "~/aavs-calibration/station/create_amp_fit_update_sql.sh \"$fit_time\" $station_id"
   ~/aavs-calibration/station/create_amp_fit_update_sql.sh "$fit_time" $station_id 
else
