...
query="select fit_time from calibration_fit order by abs(extract(epoch from (fit_time - %s))) limit 1"
...
for group in range(16):
  ...
  for subgroup_ind in range(16):
    ...
    qu_cal="select x_pha,y_pha,x_amp,y_amp,x_delay,y_delay,x_phase0,y_phase0 from calibration_solution where fit_time = %s and create_time = %s and ant_id = %s"
    cur.execute(qu_cal,(fit_time,create_time, ant_ind))
